...
<center>
<br/><br/>
<h2 style="font-size: 35px;font-weight: bold;font-family: monospace;color: blue;text-decoration: underline;">Search results for keyword: "<%=skey%>"</h2>
<table id="restable" border=1 style="margin-left: 200px;background: bisque;height: 350px;width: 350px;color: currentColor;font-family: monospace;font-size: 18px;">
<tr>
<th>Uid</th>
<th>Username</th>
<th>First Name</th>
...
<%
/////SHOW EDIT AND DELETE OPTIONS ONLY IF USER HAS PERMISSIONS
if( pedit == 1){
...
<%
//// SEARCH "user_info" TABLE USING THE KEYWORD
String srch = "select * from `user_info` where `fname` LIKE '%" + skey + "%' or `lname` LIKE '%" + skey + "%'";
...
while (rs.next()) {
	...
	////SHOW OPTIONS ONLY IF USER HAS PERMISSIONS
	if( pedit == 1){
	out.print("<td><a href=\"edit.jsp?uid=" + rs.getString("uid") + "\"> Edit</a></td>");
	}
	if( pdel == 1){
		out.print("<td><a href=\"delete.jsp?uid=" + rs.getString("uid") + "\"> Delete</a></td>");
		}
	out.println("</tr>");
}
out.print("</table>");
if(!hasRows){
	out.print("<h3>No results found.</h3><script type=\"text/javascript\">document.getElementById(\"restable\").style.display = \"none\";</script>");
}
%>
